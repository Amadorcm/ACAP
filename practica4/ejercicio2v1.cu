...
int main(void){
        ...
        // Calcular el número de bloques necesario
        ...
        for(int iter = -1; iter<numRuns; iter++){//La iteracion -1 es para que la tarjeta se ponga en marcha, normalmente siempre da peores tiempos.
                ...
                if(iter<0){
                        kernelHistograma<<<numBlocks, THREADS_PER_BLOCK>>>(dev_imagen, SIZE, dev_histo);
                }else{
